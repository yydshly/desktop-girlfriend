...
class MemorySuggestionController:
    ...
    def _on_memory_add_requested(self, event: BaseEvent) -> None:
        """Handle manual memory add request (V8-J).

        Adds a memory record directly without going through the confirmation flow.
        Does not save evidence or full original conversation.
        """
        text = event.payload.get("text")
        if not isinstance(text, str) or not text.strip():
        ...
        try:
            ...
            self._dispatch_memory_error(str(e))
            return
        except Exception:
