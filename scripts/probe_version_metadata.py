...
def _check_rc_docs() -> bool:
    ...
    if "v0.1.0-rc.0" not in content:
        print("release candidate docs: missing git tag reference")
        return False
...
def _check_product_status() -> bool:
    ...
    has_version = any(item.detail == "0.1.0-rc.0" for item in view.items)
