...
function renderRuntimeChainStatus(elements, runtime, rendererStatus = {}) {
  ...
  elements.runtimeChainStatus.textContent = [
    ...
    `speaking: ${speaking.active ? "active" : "idle"} / pending ${Boolean(speaking.pending)} / closing ${Boolean(speaking.closing)} / fallbackExpired ${Boolean(speaking.fallbackExpired)} / source ${speaking.source || "unknown"} / tts ${speaking.ttsState || "none"} / origin ${speaking.ttsSource || "unknown"} / rhythm ${speaking.rhythm || "unknown"} / mouth ${formatNumber(speaking.mouth)} / form ${formatNumber(speaking.mouthForm)}`,
